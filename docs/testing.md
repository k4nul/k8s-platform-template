...
- medium-severity gaps such as missing per-container resources, pod or container `securityContext`, per-container readiness probes, per-container liveness probes, service account token automounting on default-account workloads, mutable `latest` tags, skipped TLS verification, secret-bearing command arguments, and concrete sensitive values in rendered or bootstrap Secret templates
